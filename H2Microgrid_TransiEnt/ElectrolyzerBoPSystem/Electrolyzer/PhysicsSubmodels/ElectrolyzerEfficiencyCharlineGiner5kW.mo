...
model ElectrolyzerEfficiencyCharlineGiner5kW "Efficiency charline for Giner 5kW - still to adapt Charline parameters"
...
equation
  ...
  if use_arrayefficiency then
    ...
    eta_cl=if noEvent(P_el/P_el_n<0.1) then 0 else 0.06655*(P_el/P_el_n)^2 - 0.2741*P_el/P_el_n + 0.8709 - 0.03432/(P_el/P_el_n);
  end if;
...
<p>Equation coefficients should also be adapted, when we have access to an efficiency curve for our 5 kW Giner electrolyzer.</p>
...
end ElectrolyzerEfficiencyCharlineGiner5kW;
