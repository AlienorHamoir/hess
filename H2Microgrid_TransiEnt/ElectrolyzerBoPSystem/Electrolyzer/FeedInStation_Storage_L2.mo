...
model FeedInStation_Storage_L2
...
parameter TILMedia.VLEFluidTypes.BaseVLEFluid medium=simCenter.gasModel3 "Hydrogen model to be used" annotation (Dialog(tab="General", group="General"));
 ...
 // replaceable model  electrolyzerMassFlow = TransiEnt.Producer.Gas.Electrolyzer.Base.Physics.MassFlow.MassFlow2thOrderDynamic    annotation (Dialog(group="Electrolyzer"),__Dymola_choicesAllMatching=true); electrolyzer L2 mass flow
...
public
  ...
  TransiEnt.Basics.Interfaces.General.TemperatureIn T_set_coolant_out if useVariableCoolantOutputTemperature annotation (Placement(transformation(extent={{128,16},{88,56}})));
public
TransiEnt.Components.Boundaries.Gas.BoundaryRealGas_Txim_flow boundary_Txim_flow2(
  ...
  Modelica.Blocks.Sources.RealExpression realExpression(y=if storage.p_gas >= p_minLow_constantDemand then m_flow_hydrogenDemand_constant else 0) annotation (Placement(transformation(extent={{11,-8},{-11,8}},
        rotation=-90,
        origin={59,-76})));
  TransiEnt.Components.Gas.VolumesValvesFittings.Fittings.RealGasJunction_L2_isoth mix_H2_isoth(
  ...
  TransiEnt.Storage.Gas.GasStorage_constXi_L2 storage(
    ...
    Cspec_demAndRev_el=Cspec_demAndRev_el_other) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={54,4})));
  Electrolyzer.PEMElectrolyzer_L2 electrolyzer(
   useFluidCoolantPort=true,
   medium=medium,
   whichInput=3);

  PEMElectrolyzer_L2 electrolyzer1 annotation (Placement(transformation(extent={{-98,26},{-78,46}})));
protected
...
if integrateMassFlow then
  ...
  connect(massflowSensor_ely.m_flow, controlTotalElyStorage.m_flow_ely) annotation (Line(points={{-22.3,6},{-18,6},{-18,61},{-2,61}},  color={0,0,127},
      pattern=LinePattern.Dash));
  connect(controlTotalElyStorage.P_el_ely, electrolyzer.P_el_set) annotation (Line(points={{2,54},{2,48},{-69.8,48},{-69.8,14}}, color={0,0,127},
      pattern=LinePattern.Dash));
  ...
  if useFluidCoolantPort then
    connect(electrolyzer.fluidPortIn, fluidPortIn) annotation (Line(
      points={{-56,-7},{-56,-22},{-52,-22},{-52,-74},{-64,-74},{-64,-114},{100,-114},{100,-90}},
      color={175,0,0},
      thickness=0.5));
    connect(fluidPortOut, electrolyzer.fluidPortOut) annotation (Line(
      points={{100,-40},{100,8},{70,8},{70,20},{-50,20},{-50,-2},{-56,-2}},
      color={175,0,0},
      thickness=0.5));
  end if;
  if useHeatPort then
  connect(heat, electrolyzer.heat) annotation (Line(points={{100,-66},{74,-66},{74,-110},{-30,-110},{-30,-96},{-32,-96},{-32,-22},{-50,-22},{-50,-4.6},{-56,-4.6}},
                                                                                                         color={191,0,0}));
  end if;
  if useVariableCoolantOutputTemperature then
    connect(electrolyzer.T_set_coolant_out, T_set_coolant_out) annotation (Line(points={{-54,9},{-46,9},{-46,24},{82,24},{82,36},{108,36}},
                                                                                                                                 color={0,0,127}));
  end if;
  ...
  connect(storage.p_gas, controlTotalElyStorage.p_storage) annotation (Line(points={{49,4},{68,4},{68,65},{18,65}}, color={0,0,127}));
  connect(threeWayValve.gasPortOut1, storage.gasPortIn) annotation (Line(
      points={{16,-0.222222},{26,-0.222222},{26,18},{54,18},{54,8.9}},
      color={255,255,0},
      thickness=1.5));
  connect(valve_mFlowDes.gasPortIn, storage.gasPortOut) annotation (Line(
      points={{39.4286,-38},{39.4286,-12},{54,-12},{54,-2.3}},
      color={255,255,0},
      thickness=1.5));
  connect(boundary_Txim_flow2.gasPort, storage.gasPortOut) annotation (Line(
      points={{55,-46},{55,-34},{54,-34},{54,-2.3}},
      color={255,255,0},
      thickness=1.5));
