...
model HESS_Compressed "HESS with high-pressure compressed storage system"
  ...
  Modelica.Blocks.Interfaces.RealOutput LOH "H2 Tank Level of Hydrogen [-]"
                                                                          annotation (Placement(transformation(extent={{88,-2},{120,30}}),  iconTransformation(extent={{88,-2},{120,30}})));
  TransiEnt.Basics.Interfaces.Electrical.ElectricPowerIn P_set_FC "Input for FC power production setpoint" annotation (Placement(transformation(extent={{-118,62},{-80,102}}),iconTransformation(extent={{-118,62},{-80,102}})));
  ...
  TransiEnt.Basics.Interfaces.Electrical.ElectricPowerIn P_set_EL "Input for Electrolyzer power consumption setpoint" annotation (Placement(transformation(extent={{-118,-78},{-80,-38}}), iconTransformation(extent={{-118,-78},{-80,-38}})));
  TransiEnt.Basics.Interfaces.Electrical.ElectricPowerOut P_EL "Actual Electrolyzer electrical power balance (consumed - produced)" annotation (Placement(transformation(extent={{92,-80},{122,-50}}), iconTransformation(extent={{92,-80},{122,-50}})));
...
equation
  ...
  connect(systemEL.gasPortOut, h2StorageSystem_Compressed.H2PortIn) annotation (Line(
      ...
      color={255,255,0},
      thickness=1.5));
  connect(h2StorageSystem_Compressed.LOH, LOH) annotation (Line(points={{58,13.4},{70,13.4},{70,14},{104,14}},                 color={0,0,127}));
  connect(h2StorageSystem_Compressed.H2PortOut, H2massSink.gasPort) annotation (Line(
      points={{42,20.6},{42,44},{36,44}},
      color={255,255,0},
      thickness=1.5));
  connect(h2StorageSystem_Compressed.P_comp, systemEL.CompPower) annotation (Line(
      points={{58,-20.8},{68,-20.8},{68,-76},{-8.6,-76},{-8.6,-64}},
      color={0,135,135},
      pattern=LinePattern.Dash));
  connect(systemEL.T_env, T_environment) annotation (Line(points={{-40.6,-31.4},{-74,-31.4},{-74,0},{-100,0}}, color={0,0,127}));
  connect(PowerSetpointEL.y, systemEL.P_el_set) annotation (Line(points={{-55,-48},{-40.8,-48}},                     color={0,0,127}));
  connect(PowerOutEL.y, P_EL) annotation (Line(points={{63,-66},{88,-66},{88,-65},{107,-65}}, color={0,0,127}));
  ...
  connect(T_environment, systemFC.T_env) annotation (Line(points={{-100,0},{-74,0},{-74,78},{-41.6,78},{-41.6,77.6}},color={0,0,127}));
