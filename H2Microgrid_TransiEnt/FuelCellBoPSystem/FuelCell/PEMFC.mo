...
model PEMFC "Model of PEM Fuel Cell stack experimentally validated with Laurncelle data - model partly from Khan and Iqbal"
...
//________________________________________________________________________________//
 ...
 // Modelica.Units.SI.Voltage V_d "Dynamic activation voltage - Correa dynamic model";
  ...
  TransiEnt.Basics.Interfaces.General.TemperatureOut temperatureOut annotation (Placement(transformation(extent={{-32,-40},{-12,-20}}), iconTransformation(extent={{-32,-40},{-12,-20}})));
  ...
  Modelica.Blocks.Sources.BooleanExpression PID_T_control(y=cooling_control) annotation (Placement(transformation(extent={{10,-10},{30,10}})));
  ...
  ClaRa.Components.Utilities.Blocks.LimPID cooling_PID(
    ...
    Tau_i=tau_i,
    k=k_p,
    controllerType=Modelica.Blocks.Types.SimpleController.PI)  annotation (Placement(transformation(extent={{40,-4},{60,16}})));
  Modelica.Blocks.Sources.RealExpression Q_Cooling(y=Q_flow_cooling) "excess waste heat generated by fuel cell system, actively cooled by default" annotation (Placement(transformation(extent={{-20,-80},{0,-60}})));
  TransiEnt.Basics.Interfaces.Thermal.HeatFlowRateOut excessHeatFlowOut annotation (Placement(transformation(extent={{-32,-70},{-12,-50}})));

  Modelica.Blocks.Sources.RealExpression T_setpoint(y=T_nom) annotation (Placement(transformation(extent={{10,8},{30,28}})));
equation
  // _____________________________________________
  //
 ...
 // Minimal load current management
  ...
  if is_Shutdown then
    ...
    I = 0;
    V_stack = no_Cells * 1.23; // replace E_0 by OCV
    V_act = 0;
 ...
 // temperature design flow direction
  ...
  connect(temperatureOut,T_op_out. y) annotation (Line(points={{-22,-30},{4,-30},{4,-40},{-1,-40}},
                                                                                    color={0,0,127}));
  connect(PID_T_op.y, cooling_PID.u_m) annotation (Line(points={{31,-14},{50.1,-14},{50.1,-6}},                   color={0,0,127}));
  connect(PID_T_control.y, cooling_PID.activateInput) annotation (Line(points={{31,0},{32,-2},{38,-2}},          color={255,0,255}));
  connect(excessHeatFlowOut, Q_Cooling.y) annotation (Line(
      points={{-22,-60},{4,-60},{4,-70},{1,-70}},
      color={162,29,33},
  ...
  connect(T_setpoint.y, cooling_PID.u_s) annotation (Line(points={{31,18},{36,18},{36,14},{38,14},{38,6}},       color={0,0,127}));
